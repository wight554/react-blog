...
class Post extends Component {
  ...
  addComment = async val => {
    let newComment = await this.props.createElement(`posts/${this.props.match.params.id}/comments/`, {name: val, author: this.props.auth.user.user.username});
  ...
  editComment = async comment => {
    const updatedComment = await this.props.updateElement(`posts/${this.props.match.params.id}/comments/${comment._id}`, comment.name);
    const comments = this.state.comments.map(c =>
      (c._id === updatedComment._id)
      ? {...c, name: updatedComment.name}
      : c
    )
    this.setState({comments: comments});
  }
      ...
      <div className='Post'>
      {
        ...
        <>
          <header>
            ...
            <span>{this.state.author}, {this.state.date.toString().slice(0, 10)}</span>
          </header>
          ...
          :
            ...
            {
              this.props.auth.isAuthenticated && this.state.author === this.props.auth.user.user.username &&
...
export default connect(
  mapStateToProps,
  { getElement, createElement, removeElement, updateElement }
)(Post);
